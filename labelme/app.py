...
from labelme.label_file import LabelCocoFile
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        saveCoco = action(
            self.tr("&Export Coco"),
            self.exportCoco,
            shortcuts["export_coco"],
            "save",
            self.tr("Export COCO JSON format"),
        )
    ...
    # enable save and save coco when change the labels
    def setDirty(self):
    ...
    def saveCocoFormat(self, foldername):
        lc = LabelCocoFile()
        lc.load_json_file(foldername)
    ...
    def exportCoco(self, _value=False):
        assert not self.image.isNull(), "cannot export empty image"
        if self.labelFile:
            print(' ------- labelfile -------')
            self._exportCoco(self.lastOpenDir)
        elif self.output_file:
            self._exportCoco(self.output_file)
            self.close()
        else:
            self._exportCoco(self.saveFileDialog())
    ...
    def _exportCoco(self, foldername):
        print(foldername, ' ------- folderanem -----')
        if foldername:
            self.saveCocoFormat(foldername)
